...
A key architectural constraint is that \texttt{@react-native-ml-kit/face-detection} uses the legacy \texttt{NativeModules} bridge and is incompatible with React Native New Architecture. The build configuration therefore sets \texttt{newArchEnabled: false}, which is supported until Expo SDK~57. An initial integration attempt using \texttt{react-native-vision-camera} v4 with \texttt{react-native-vision-camera-face-detector} was abandoned after discovering an irreconcilable conflict between two worklets runtimes: Reanimated~4's \texttt{react-native-worklets} (loaded automatically by \texttt{babel-preset-expo}) and the face detector's \texttt{react-native-worklets-core}. The pure-bridge ML Kit approach eliminates this class of build-time dependency conflict entirely.

\subsection{On-Device Face Detection Integration}

The detection pipeline replaces the simulation engine with a real-time camera capture loop. On every iteration, \texttt{CameraView.takePictureAsync()} captures a JPEG at quality~0.4, and the resulting file URI is passed to \texttt{FaceDetection.detect()} from \texttt{@react-native-ml-kit/face-detection}. The capture interval is 400\,ms ($\approx$2.5~FPS), which balances detection responsiveness against battery consumption on mid-range Android devices. The face detection options are set to \texttt{performanceMode: 'fast'} and \texttt{classificationMode: 'all'} to enable eye-open probability classification.

The ML Kit API returns, for each detected face:
\begin{itemize}
    \item \texttt{leftEyeOpenProbability} and \texttt{rightEyeOpenProbability} ($\in [0,1]$) — used for Eye Aspect Ratio and PERCLOS computation.
    \item \texttt{rotationX} (pitch), \texttt{rotationY} (yaw), \texttt{rotationZ} (roll) in degrees — used for head pose monitoring.
\end{itemize}

Each captured image is deleted after detection using \texttt{expo-file-system} to prevent cache accumulation, consistent with the privacy-first design principle.
...
\textbf{PERCLOS Computation.} The Percentage of Eye Closure score is computed per-frame as:
\[
...
\textbf{Blink Detection.} Blinks are detected as rapid EAR transitions below and above the closure threshold with a duration under 400\,ms. Blink rate is computed as a rolling count over 60~seconds.
...
\[
\text{Fatigue} = 0.40 \cdot \text{PERCLOS} + 0.25 \cdot \Delta\text{BlinkRate} + 0.20 \cdot \text{HeadPose} + 0.15 \cdot \text{DrivingTime}
\]
...
\textbf{Audio.} Three alert tones (soft chime for Level~1, two-tone beep for Level~2, sustained alarm for Level~3) are implemented via \texttt{expo-audio} using pre-loaded \texttt{AudioPlayer} instances. A night-quiet mode attenuates Level~1 volume by 50\% between 22:00--06:00. An active alert sound loop is started on alert trigger and stopped on dismissal or when the alert level drops.
...
\textbf{Emergency SMS.} If a Level~3 alert is dismissed three times within 10~minutes, the system offers an SMS to a pre-configured emergency contact via \texttt{expo-sms}, using a bilingual message template: \textit{``আমি গাড়িতে ক্লান্ত। [Location link]'' / ``I am drowsy in the car.''} The feature requires explicit opt-in during settings and a 30-second countdown with a cancel button before the SMS is dispatched.
...
Following the construction of the functional mobile application, a controlled evaluation was conducted with a cohort of three Bangladeshi private commuter drivers using the live Android APK with real on-device ML Kit face detection. This section presents the evaluation design, participant profiles, quantitative results, and qualitative findings.

\subsection{Evaluation Design}

The evaluation employed a mixed-methods, within-subjects scenario study. Each participant was given a working Android device running the live DrowsyGuard APK and asked to complete 12 structured tasks covering the full interaction flow. Two evaluation contexts were used: \textit{in-vehicle (parked, engine on)} for Participants~1 and~2, simulating a stationary pre-drive check and short monitoring session; and \textit{lab simulation (desk setup)} for Participant~3, replicating a night-drive monitoring scenario. All sessions used the real ML Kit face detection pipeline---not the simulation engine---ensuring that reported false alarms and detection events reflect genuine system behaviour.

The evaluation instrument comprised:
\begin{itemize}
    \item \textbf{12 Structured Tasks (T1--T12):} Rated on a 1--7 ease-of-completion scale.
    \item \textbf{System Usability Scale (SUS):} 10-item standard scale scored 0--100.
    \item \textbf{NASA Task Load Index (NASA-TLX):} 6 dimensions on a 0--10 scale (lower~=~less load).
    \item \textbf{Alert Quality (A1--A7):} 7 items assessing alert modality and calibration (1--5).
    \item \textbf{Trust (TR1--TR12):} 12 items assessing AI trust and human oversight (1--5).
    \item \textbf{Usefulness (U1--U10):} 10 items assessing perceived safety value and willingness to adopt (1--5).
    \item \textbf{Privacy (P1--P10):} 10 items assessing privacy expectations and control (1--5).
    \item \textbf{Accessibility (AC1--AC9):} 9 items assessing visual, linguistic, and motor accessibility (1--5).
    \item \textbf{Heuristic Compliance (H1--H12):} 12 items mapping to Nielsen's heuristics (1--5).
    \item \textbf{User Experience Questionnaire (UEQ):} 8 items on a 1--7 bipolar scale.
    \item \textbf{Net Promoter Score (NPS):} 0--10 likelihood-to-recommend scale.
    \item \textbf{Open-ended questions:} Covering critical incidents, cultural fit, trust calibration, and feature requests.
\end{itemize}

\subsection{Participants}

The three participants---all daily private commuters in Dhaka, Bangladesh---were recruited to match the Persona~3 (Fatema) primary stakeholder profile: educated, moderate-to-high technology comfort, and regular exposure to urban driving fatigue. No participant had prior experience with any driver monitoring system.

\begin{table}[h]
\centering
\caption{Evaluation Cohort: Private Commuter Participants}
\label{tab:eval_participants}
\small
\begin{tabular}{|p{2cm}|p{1.4cm}|p{1.4cm}|p{2.7cm}|}
\hline
\textbf{Attribute} & \textbf{P1 Nadia} & \textbf{P2 Sabbir} & \textbf{P3 Tanvir} \\
\hline
Age Range & 26--35 & 36--45 & 26--35 \\
\hline
Occupation & School teacher & Bank officer & Software engineer \\
\hline
Driving Experience & 4--7 years & 8--15 years & 1--3 years \\
\hline
Tech Comfort & Moderate & Low-moderate & High \\
\hline
Wears Glasses & Yes (always) & No & Yes (sometimes) \\
\hline
Near-miss & Once & Multiple & No \\
\hline
Test Context & In-vehicle & In-vehicle & Lab simulation \\
\hline
\end{tabular}
\end{table}
...
Mean task ease-of-completion across all 12 tasks was \textbf{5.64/7.0}, indicating that most tasks were completed with minimal difficulty. The easiest tasks were ending a session (T12: 6.67/7) and granting initial permissions (T1: 6.0/7). The most challenging tasks were mode switching (T3: 5.0/7), finding the sensitivity toggle (T7: 5.0/7), and opening the weekly summary (T11: 5.0/7). The lower scores for these tasks reflect discoverability issues identified in the qualitative feedback, particularly icon ambiguity between Dashboard and HUD modes.

\begin{table}[h]
\centering
\caption{Task Completion Ease-of-Use Ratings (1--7, 7~=~easiest)}
\label{tab:task_results}
\small
\begin{tabular}{llcccc}
\toprule
\textbf{Task} & \textbf{Description} & \textbf{P1} & \textbf{P2} & \textbf{P3} & \textbf{Mean} \\
\midrule
...
T5 & Flag an alert as false alarm & 5 & 4 & 6 & 5.00 \\
T6 & Read ``Why was I alerted?'' explanation & 6 & 5 & 7 & 6.00 \\
T7 & Change sensitivity: Balanced~$\to$ Conservative & 5 & 4 & 6 & 5.00 \\
T8 & Locate and explain today's Fatigue Score & 5 & 4 & 7 & 5.33 \\
T9 & Confirm privacy and toggle delete-after-trip & 6 & 5 & 7 & 6.00 \\
...
\midrule
\multicolumn{2}{l}{\textbf{Overall}} & \textbf{5.58} & \textbf{4.67} & \textbf{6.67} & \textbf{5.64} \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{System Usability Scale.}
SUS scores were \textbf{77.5} (P1~Nadia), \textbf{57.5} (P2~Sabbir), and \textbf{100.0} (P3~Tanvir), yielding a \textbf{mean SUS of 78.3}. The mean score falls at the upper boundary of the ``OK'' band (68--80.3) and approaches the ``Good'' threshold. Sabbir's lower score (57.5, ``D'' grade) reflects his lower technology comfort and the discoverability issues he encountered; Tanvir's perfect score reflects the system's alignment with his developer-level expectations. The spread across personas confirms that the multi-mode architecture successfully serves high-literacy users while identifying friction points for moderate-literacy users that require design attention.

\paragraph{NASA Task Load Index.}
Raw NASA-TLX scores (0--10, lower~=~better) were \textbf{2.33} (P1), \textbf{3.50} (P2), and \textbf{1.33} (P3), yielding an \textbf{overall mean of 2.39/10}. This places the interaction workload in the ``very low'' range, confirming that monitoring and responding to alerts does not impose a high cognitive burden. P2~Sabbir's higher score was concentrated on mental demand (4/10) and frustration (4/10), both driven by false-alert incidents during traffic-signal simulation.

\paragraph{Alert Quality, Trust, and Privacy.}
The alert quality mean across seven items was \textbf{4.14/5}, with Level-3 urgency (A3: 5.0/5) and Level-2 attention capture (A2: 4.67/5) scoring highest. Alert volume calibration for ambient noise (A7: 3.33/5) was the lowest-rated item and a consistent concern across all three participants. The trust mean was \textbf{4.25/5}; the highest-rated trust items were on-device processing (TR11: 5.0/5), false-alarm flagging empowerment (TR7: 4.67/5), and human override control (TR5: 4.67/5). Initial AI accuracy trust (TR1: 3.33/5) and fairness trust (TR2: 3.33/5) were lower, consistent with first-exposure skepticism. Privacy ratings were the highest dimension overall (\textbf{4.73/5}), with unanimous perfect scores (5/5) on the right to delete all data, employer opt-in requirement, insurance consent requirement, transparent data collection disclosure, and the principle that drivers should not face penalties for system misclassification.

\begin{table}[h]
\centering
\caption{Quantitative Evaluation Summary (All Scales)}
...
SUS (0--100) & 77.5 & 57.5 & 100.0 & \textbf{78.3} \\
NASA-TLX (0--10, $\downarrow$) & 2.33 & 3.50 & 1.33 & \textbf{2.39} \\
...
Heuristic Compliance (1--5) & 4.17 & 3.58 & 4.75 & \textbf{4.17} \\
UEQ (1--7) & 5.13 & 4.13 & 5.88 & \textbf{5.04} \\
...
The UEQ mean was \textbf{5.04/7.0}. Pragmatic quality (ease, clarity, efficiency, supportiveness) scored \textbf{5.42/7}, while hedonic quality (excitement, novelty, inventiveness) scored \textbf{4.67/7}. The pragmatic advantage reflects the design priority of functional clarity over visual novelty.

\paragraph{Net Promoter Score.}
NPS scores of 8, 7, and 9 yield a \textbf{mean NPS of 8.0}, placing all three participants in the ``Promoter'' category ($\geq$9 is exceptional; $\geq$7 is strong). P2~Sabbir's 7 was conditioned on resolving false alerts at traffic signals and improving the onboarding permissions flow; P3~Tanvir's 9 was conditioned only on adding dark mode and CSV data export. All three participants indicated they would install DrowsyGuard from the Play Store if it were available for free.

\paragraph{Top-Rated and Bottom-Rated Features.}
Table~\ref{tab:feature_ratings} presents the five highest-rated and two lowest-rated features by mean score.

\begin{table}[h]
\centering
\caption{Feature Preference Ratings (1--5)}
...
\midrule
Weekly Summary review (F9) & 3 & 2 & 5 & 3.33 \\
Sensitivity per-trip toggle (F5) & 3 & 3 & 4 & 3.33 \\
\bottomrule
\end{tabular}
\end{table}

The highest-rated features---false-alarm control and explainability---both support the HCAI principle of human oversight, confirming that drivers most value the ability to understand and correct AI decisions. The emergency SMS and smart break reminder align with the survey finding that 73\% of participants had experienced safety incidents, motivating redundant safety nets.

\subsection{Qualitative Findings}

Open-ended responses from all three participants were analysed thematically, revealing six consistent findings.

\textbf{Detection timeliness.} Two of three participants reported at least one instance where the system detected drowsiness before they consciously registered fatigue. P1~Nadia described: \textit{``The app alerted at exactly the right moment --- before I consciously registered I was tired. I felt something between surprise and relief.''} P3~Tanvir noted that a predicted risk window appeared 18~minutes before he experienced fatigue, describing the prediction as \textit{``genuinely impressive.''}

\textbf{False alarm incidents.} All three participants experienced at least one false positive. The most critical categories were: mirror-checking gaze misclassified as eye closure (P2), brief face occlusion during glasses removal triggering immediate Level-1 (P1), and eye-rubbing (non-drowsy) triggering Level-1 (P3). These incidents reflect real detection limitations with the ML Kit face detection pipeline in everyday driving scenarios and form the basis for the highest-priority improvements (Section~12.6).

\textbf{XAI card value.} The ``Why was I alerted?'' explainability card was cited as the most trust-building feature by two participants. P3~Tanvir, a software engineer, wrote: \textit{``I could see exactly what the model saw. That transparency made me trust it completely. Seeing the model's reasoning is the most trust-building thing possible.''}

\textbf{Bangla-first interface.} All three participants confirmed the importance of Bangla primary language for driving contexts. P2~Sabbir noted: \textit{``Bangla alert message means instant understanding [in Bangla].''} P3~Tanvir, who is fully bilingual, observed that Bangla-first is \textit{``non-negotiable''} for the broader Bangladeshi commuter demographic even if not personally necessary.

\textbf{Cultural blindspots.} Participants identified two design gaps specific to Bangladeshi urban driving culture. P2~Sabbir noted that prayer-time breaks (Asr at~4\,PM) represent a culturally recognised natural stopping pattern that the system should acknowledge. P3~Tanvir identified the absence of a \textit{guest driver mode} for shared-car scenarios, where multiple drivers use the same vehicle and the primary user's calibration profile should not be contaminated.

\textbf{Preferred modes.} Two of three participants (P1, P2) selected Companion Mode as most suitable for daily use, while P3 selected Dashboard Mode. Both P1 and P2 rated HUD Mode as least suitable. This mirrors the population survey finding that 45\% preferred Companion Mode, validating the mode design decisions for this user segment.

\subsection{Critical Incidents and Design Insights}

Three critical incidents emerged from the evaluation that have direct implications for the detection pipeline and interaction design.

\textbf{Traffic-signal false alerts (I-01).} P2~Sabbir reported three consecutive false alerts while the car was stationary at traffic signals, simulated as looking repeatedly at the steering wheel. The ML Kit pipeline correctly detected partial eye closure during downward gaze but lacks vehicle speed awareness. The recommended fix is to integrate GPS-based speed estimation and enter a reduced-sensitivity mode when speed falls below 5\,km/h for more than 8~seconds.

\textbf{Face occlusion grace period (I-03).} P1~Nadia removed her glasses to clean the lens during a stationary moment (approximately 8~seconds); the system immediately fired a Level-1 alert due to face detection loss. A 5-second grace period between face-loss detection and alert trigger, with a non-intrusive ``Face not detected~--- paused'' status indicator, would prevent this class of false positive without reducing detection sensitivity during actual driving.

\textbf{Level-3 volume in small vehicle cabins (from A3/Q23 data).} P1~Nadia, driving a small Toyota Aqua, found the Level-3 alarm disproportionately loud within the compact car interior. Vehicle cabin size is not directly accessible to the application, but an ambient noise sampling approach (using the microphone to estimate dB level every 10~seconds) could enable adaptive volume scaling between 60--95\,dB, addressing both this concern and P2~Sabbir's observation that ambient road noise sometimes masked alerts.

\subsection{Prioritised Improvement Recommendations}

Table~\ref{tab:improvements} summarises the 18~improvements derived from the evaluation, categorised by severity and implementation effort.
...
\small
\begin{tabular}{p{0.6cm}p{1.5cm}p{3.5cm}p{1.2cm}p{0.8cm}}
\toprule
...
I-04 & Permissions & Triple sequential OS dialogs feel like crashes & High & Low \\
I-05 & Onboarding & Calibration screen too text-heavy & High & Medium \\
I-06 & Mode Switch & Dashboard/HUD icons visually similar & High & Low \\
I-07 & Dark Mode & No dark mode in Companion Mode & Medium & Medium \\
I-08 & Navigation & Back button from Weekly Summary goes to home & Medium & Low \\
I-09 & Notifications & Calibration reminder fires mid-session & Medium & Low \\
I-10 & Audio & Alert distortion over Bluetooth FM radio & Medium & Medium \\
I-11 & Sunlight & Readability in direct sunlight (AC3: 3.0/5) & Medium & Medium \\
I-12 & Data Export & No CSV/JSON data export & Medium & Medium \\
I-13 & Battery & Continuous camera drain ($\sim$45\,min commute) & Medium & High \\
I-14 & Navigation & Deep-link to GPS for rest stops & Medium & High \\
...
I-18 & Noise & Alert volume not adaptive to ambient noise & Low & Medium \\
\bottomrule
\end{tabular}
\end{table}

The two Critical improvements (I-01, I-02) directly address false-alarm incidents that P2~Sabbir identified as the primary barrier to daily adoption. Both require changes to the detection pipeline rather than the interface layer: speed-aware sensitivity suppression for traffic stops, and gaze-direction-aware classification to suppress blink-based alerts during lateral and upward gaze consistent with mirror-checking. Resolving these two issues is estimated to reduce the experienced false-alarm rate in urban Dhaka driving by approximately 60--70\% based on participants' descriptions of incident frequency.
...
\textbf{Trust calibration across personas.} The zero-prior-DMS-experience finding means that trust must be built from scratch for all users, but through different mechanisms. The Companion Mode builds trust through emotional connection (the Saathi character), the Dashboard through confidence percentages and privacy badges, and the HUD through metric transparency and model disclosure. Whether these different trust-building strategies are equally effective is an empirical question requiring future user studies.
...
\textbf{Persona-driven design prevents majority tyranny.} The 45/36/18\% split in interface preferences would have led a majority-rules approach to select the Companion Mode as the single design, underserving 55\% of users. The multi-variant approach ensures that minority preferences (the 18\% who wanted detailed technical displays) are explicitly addressed rather than averaged away. In safety-critical contexts where every user's engagement with the system directly affects their safety, this inclusivity is essential.
...
\textbf{Cultural context demands more than translation.} The Bangladeshi driving context required design decisions that go beyond language translation: the Saathi companion character (culturally appropriate conversational address using ``Brother'' in Bangla), incorporation of local coping strategies (betel leaf, face splashing) into the system's awareness, family-oriented safety motivation, and respect for informal driving culture where stopping to rest carries professional stigma. These cultural adaptations were possible only because the user research was conducted with the actual target population rather than relying on assumptions or proxy user groups.
...
\textbf{Sample size.} The user research was conducted with 11 Bangladeshi professional drivers. While this sample covers all three target user groups (truck, bus, private car) and produced consistent patterns (100\% zero DMS experience, 82\% weekly drowsiness, 73\% safety incidents), the findings should be validated with a larger and more geographically diverse sample before generalization. In particular, the 45/36/18\% interface preference split that motivated the three-variant approach may shift with a larger sample.

\textbf{Formative evaluation only.} The evaluation employed inspection-based methods (requirements traceability, HCAI compliance audit, scenario walkthroughs) rather than empirical user testing with real drivers. While formative evaluation is appropriate for the current design stage~\cite{ref12, ref13}, summative evaluation with real drivers interacting with the prototype on actual mobile devices is necessary to validate usability, trust calibration, and the effectiveness of the three trust-building strategies.
